          ...
          <div class="col-lg-8">
            ...
            <section class="card card-body border-0 shadow-sm p-4 mb-4" id="basic-info">
              ...
              <div class="col-sm-6 mb-3">
                ...
                <select class="form-select" id="place_type" name="place_type" value="${place.place_act}" required>
                  <option value="" disabled>유형을 선택하세요</option>
                  <option value="랜드마크" <c:if test="${place.place_act == '랜드마크'}">${'selected'} </c:if>>랜드마크</option>
                  <option value="야외활동" <c:if test="${place.place_act == '야외활동'}">${'selected'} </c:if>>야외활동</option>
                  <option value="전시" <c:if test="${place.place_act == '전시'}">${'selected'} </c:if>>전시</option>
                  <option value="유적지" <c:if test="${place.place_act == '유적지'}">${'selected'} </c:if>>유적지</option>
                  <option value="쇼핑" <c:if test="${place.place_act == '쇼핑'}">${'selected'} </c:if>>쇼핑</option>
              ...
              <div class="col-sm-6 mb-3">
                ...
                <select class="form-select" id="place_act" name="place_act" value="${place.place_type}" required>
                  <option value="" disabled>세부 유형을 선택하세요</option>
                  <option value="공원" <c:if test="${place.place_type == '공원'}">${'selected'} </c:if>>공원</option>
                  <option value="궁궐" <c:if test="${place.place_type == '궁궐'}">${'selected'} </c:if>>궁궐</option>
                  <option value="미술관" <c:if test="${place.place_type == '미술관'}">${'selected'} </c:if>>미술관</option>
                  <option value="박물관" <c:if test="${place.place_type == '박물관'}">${'selected'} </c:if>>박물관</option>
                  <option value="지역명소" <c:if test="${place.place_type == '지역명소'}">${'selected'} </c:if>>지역명소</option>
                  <option value="수목원" <c:if test="${place.place_type == '수목원'}">${'selected'} </c:if>>수목원</option>
                  <option value="복합문화공간" <c:if test="${place.place_type == '복합문화공간'}">${'selected'} </c:if>>복합문화공간</option>
                  <option value="백화점" <c:if test="${place.place_type == '백화점'}">${'selected'} </c:if>>백화점</option>
                  <option value="전통시장" <c:if test="${place.place_type == '전통시장'}">${'selected'} </c:if>>전통시장</option>
