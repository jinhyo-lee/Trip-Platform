...
<section class="container mb-5 pb-1">
	...
	<div class="row">
		<div class="tab-content col-md-7 mb-md-0 mb-4">
			<div class="tab-pane fade show active" id="review-list" role="tabpanel">
				<div class="mb-4 pb-4 border-bottom">
					...
					<c:if test="${!empty sessionScope.id}">
						<button class="btn btn-outline-primary mb-sm-0 mb-3" href="#modal-review" data-bs-toggle="modal" onclick="writes(${room_id},${page})">
				...
				<!-- Review list -->
				<c:forEach var="roomReview" items="${reviewlist}" varStatus="status">
				<div class="mb-4 pb-4 border-bottom">
					<div class="d-flex justify-content-between mb-3">
						...
						<div class="d-inline-block">
							...
							<span class="text-muted fs-sm">
								<button style="padding: 0;" type="button" class="btn btn-link btn-sm" title="좋아요 추가" onclick="location.href='roomReviewLike?room_rev_id=${roomReview.room_rev_id}&room_id=${room.room_id}&page=${page}&state=cont'">
							...
							<span class="text-muted fs-sm">
								<button style="padding: 0;" type="button" class="btn btn-link btn-sm" title="좋아요 삭제" onclick="location.href='roomReviewLike?room_rev_id=${roomReview.room_rev_id}&room_id=${room.room_id}&page=${page}&state=cont'">
							...
							<div class="dropdown d-inline-block" style="float: right;">
								...
								<div class="dropdown-menu dropdown-menu-end my-1">
									<button class="btn btn-link btn-sm" type="button" href="#modal-reply" data-bs-toggle="modal" onclick="reply(${roomReview.room_rev_id },${page },${roomReview.room_id })">
										<i class="fi-reply fs-lg me-2"></i><span class="fw-normal">댓글</span>
									</button>
									<c:if test="${roomReview.id == sessionScope.id}">
									<button class="btn btn-link btn-sm" type="button" id="test1" href="#modal-review-modify" data-bs-toggle="modal" onclick="modify(${roomReview.room_rev_id },${room_id },${page })">
										<i class="fi-edit fs-lg me-2"></i><span class="fw-normal">수정</span>
									</button>
									</c:if>
									<c:if test="${roomReview.id == sessionScope.id}">
									<button type="button" value="삭제" class="btn btn-link btn-sm" onclick="location='roomReviewDelete?room_rev_id=${roomReview.room_rev_id}&room_id=${room_id }&page=${page }'">
					...
					<a class="collapse-label collapsed" href="#seeMoreOverview_rep${roomReview.room_rev_id}" data-bs-toggle="collapse" data-bs-label-collapsed="댓글 보기" data-bs-label-expanded="접기" role="button" aria-expanded="false" aria-controls="seeMoreOverview"
						onclick="replyList(${roomReview.room_rev_id },${room_id },${page })"></a>
					<!-- Reply List -->
					<div class="collapse" id="seeMoreOverview_rep${roomReview.room_rev_id}">
						<!-- load area -->
					</div>
				</div>
				</c:forEach>
			...
			<%@ include file="../room_qna/room_qna_list.jsp"%>
			</div>
		</div>
	</div>
</section>